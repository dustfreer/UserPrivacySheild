...
public class PackagesInfo {
	...
	// private List<AppInfoUtil> mappList;
	...
	public List<AppInfoUtil> getRunningProcess() {
		...
		List<AppInfoUtil> list = new ArrayList<AppInfoUtil>();
		List<AppInfoUtil> slist = new ArrayList<AppInfoUtil>();
		for (RunningAppProcessInfo ra : run) {
			...
			if (getInfo(ra.processName) != null) {
				if ((getInfo(ra.processName).flags & ApplicationInfo.FLAG_SYSTEM) == 0
					...
					AppInfoUtil pr = new AppInfoUtil();
					pr.setAppIcon(getInfo(ra.processName).loadIcon(pm));
					pr.setAppName(getInfo(ra.processName).loadLabel(pm)
							.toString());
