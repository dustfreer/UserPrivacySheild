...
import com.security.ass.pinyinime.InputModeSwitcher.ToggleStates;
...
public class SoftKeyboard {
	...
	/**
	 * The width of the soft keyboard. 键盘的宽庿
	 */
	private int mSkbCoreWidth;

	/**
	 * The height of the soft keyboard. 键盘的高庿
	 */
	...
	/** The left and right margin of a key. 丿个按键的左右间隔 */
	private float mKeyXMargin = 0;

	/** The top and bottom margin of a key. 丿个按键的上下间隔 */
	...
	/**
	 * 根据坐标查找按键，如果坐标在某个按键区域内，就返回这个按键，如果坐标不在承有的按键区域内，返回离它朿近的按键?
	 * 
	 * @优化 可以在判断坐标在某个按键区域内的时忙，并且加上判断离它朿近的按键，这样就只需要一次遍历就行了?
	 * @param x
	 * @param y
	 * @return
	 */
	public SoftKey mapToKey(int x, int y) {
		...
		for (int row = 0; row < rowNum; row++) {
			...
			for (int i = 0; i < keyNum; i++) {
				SoftKey sKey = softKeys.get(i);
				int disx = (sKey.mLeft + sKey.mRight) / 2 - x; // 按键x坐标的中心点到x之间的距禿
				int disy = (sKey.mTop + sKey.mBottom) / 2 - y; // 按键y坐标的中心点到y之间的距禿
	...
	/**
	 * 改变键盘的状态，并且根据键盘状濁中的mKeyStates[]来设置每个按键?
	 * 
	 * @param toggleStates
	 *            整个键盘的状怿
	 */
