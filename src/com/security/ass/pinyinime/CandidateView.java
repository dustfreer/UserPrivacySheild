...
public class CandidateView extends View {
	/**
	 * The minimum width to show a item. 丿个item朿小的宽度
	 */
	private static final float MIN_ITEM_WIDTH = 22;

	/**
	 * Suspension points used to display long items. 省略叿
	 */
	private static final String SUSPENSION_POINTS = "...";

	/**
	 * The width to draw candidates. 候?词区域的宽庿
	 */
	private int mContentWidth;

	/**
	 * The height to draw candidate content. 候?词区域的高庿
	 */
	...
	/**
	 * Balloon hint for candidate press/release. 当忙?词被按下的时忙显示的气泡
	 */
	private BalloonHint mBalloonHint;

	/**
	 * Desired position of the balloon to the input view. 气泡显示的位罿
	 */
	 ...
	 * arrows. 箭头更新接口。在onDraw（）中，当mUpdateArrowStatusWhenDraw为true＿
	 * 该接口的updateArrowStatus（）方法被调用?因为箭头是放在候?词集装箱中的，不是放在候?词视图中?
	 */
	...
	/**
	 * Page number of the page displayed in this view. 候?词视图显示的页砿
	 */
	private int mPageNo;

	/**
	 * Active candidate position in this page. 活动（高亮）的忙?词在页面的位置?
	 */
	 ...
	 * string), the highlight in candidate view should be removed. 是否高亮活动的忙?词
	 */
	private boolean mEnableActiveHighlight = true;

	/**
	 * The page which is just calculated. 刚刚计算的页砿
	 */
	...
	/**
	 * The Drawable used to display as separators between candidates. 分隔符图牿
	 */
	private Drawable mSeparatorDrawable;

	/**
	 * Color to draw normal candidates generated by IME. 正常候?词的颜色，来自输入法词库的候?词?
	 */
	private int mImeCandidateColor;

	/**
	 * Color to draw normal candidates Recommended by application.
	 * 推荐候?词的颜色，推荐的忙?词是来自APP的?
	 */
	 ...
	 * 候?词的文本大小，它可以是 mImeCandidateTextSize 咿 mRecommendedCandidateTextSize
	 * 其中的一个?
	 */
	private int mCandidateTextSize;

	/**
	 * Paint used to draw candidates. 候?词的画笿
	 */
	private Paint mCandidatesPaint;

	/**
	 * Used to draw footnote. 附注的画笿
	 */
	...
	/**
	 * Rectangle used to draw the active candidate. 活动（高亮）候?词的区埿
	 */
	 ...
	 * allocated to each candidates. 候?词的左边和右边间隔
	 */
	private float mCandidateMargin;

	/**
	 * Left and right extra margins for a candidate. 候?词的左边和右边的额外间隿
	 */
	private float mCandidateMarginExtra;

	/**
	 * Rectangles for the candidates in this page. 在本页忙?词的区域向量列衿
	 **/
	private Vector<RectF> mCandRects;

	/**
	 * FontMetricsInt used to measure the size of candidates. 候?词的字体测量对豿
	 */
	private FontMetricsInt mFmiCandidates;

	/**
	 * FontMetricsInt used to measure the size of footnotes. 附注的字体测量对豿
	 */
	...
	/**
	 * 根据候?词的来源设置忙?词使用的颜色和文本大小，并计算省略号的宽度?
	 * 
	 * @param decInfo
	 */
	...
	/**
	 * 计算候?词区域的宽度和高度、忙?词文本大小、附注文本大小?省略号宽度。当尺寸发生改变时调用?在onMeasure（）中调用?
	 */
	...
	/**
	 * 对还没有分页的忙?词进行分页，计算指定页的忙?词左右的额外间隔?
	 * 
	 * @param pageNo
	 * @return
	 */
	private boolean calculatePage(int pageNo) {
		...
		for (int p = fromPage; p <= pageNo; p++) {
			...
			if (!onlyExtraMargin) {
				// pageNo是最后一页或者往后的丿页，这里应该就是对忙?词进行分页的地方，保证每页候?词都能正常显示?
	...
	/**
	 * 返回坐标点所在或者离的最近的候?词区域在mCandRects的索弿
	 * 
	 * @param x
	 * @param y
	 * @return
	...
	/**
	 * 候?词视图触摸事件处理。在候?词集装箿 CandidatesContainer 中的触摸事件处理函数中调用?
	 * 
	 * @param event
	 * @return
	 */
	...
	/**
	 * 按下某个候?词的定时器。主要是刷新页面，显示按下的候?词为高亮状态?
	 * 
	...
	private class PressTimer extends Handler implements Runnable {
		...
		private int mPageNoToShow; // 显示的页砿
		private int mActiveCandOfPage; // 高亮候?词在页面的位置
