...
public class SoftKeyToggle extends SoftKey {
	...
	public boolean enableToggleState(int stateId, boolean resetIfNotFound) {
		...
		if (stateId > 0) {
			/*
			 * 先设置判断的stateId为当前mKeyMask，然后在getToggleState（）中根据mKeyMask进行查找，如果找到了＿
			 * 就返回true，如果没有找刿
			 * ，就重置mKeyMask中的KEYMASK_TOGGLE_STATE位为0，再判断resetIfNotFound是否为true＿
	 ...
	 * KEYMASK_TOGGLE_STATE是否与stateId时相等的，如果不是就移动到下丿个ToggleState再找
	 * ，直到与stateId时相等或者没有下丿个ToggleState为止?
	 * 
	 * @return
	 */
	...
	/**
	 * 按键变化的状态，每一个按键的变化状濁都是一个单向列表，mNextState指向丿下个变化的状态?
	 * 
