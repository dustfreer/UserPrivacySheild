...
public class InputModeSwitcher {
	...
	/**
	 * User defined key code, used by soft keyboard. 微笑按键的code，比如中文时，语訿键的上面那个按键?
	 */
	...
	/**
	 * A kind of soft keyboard layout. An input mode should be anded with
	 * {@link #MASK_SKB_LAYOUT} to get its soft keyboard layout. 指明微笑软键盿
	 */
	private static final int MASK_SKB_LAYOUT_SMILEY = 0x40000000;

	/**
	 * A kind of soft keyboard layout. An input mode should be anded with
	 * {@link #MASK_SKB_LAYOUT} to get its soft keyboard layout. 指明电话软键盿
	 */
	...
	/**
	 * Mode for inputing Chinese with soft keyboard. 中文标准软键盘模弿
	 */
	...
	/**
	 * Mode for inputing English lower characters with soft keyboard. 英文小写软键盘模弿
	 */
	public static final int MODE_SKB_ENGLISH_LOWER = (MASK_SKB_LAYOUT_QWERTY
			| MASK_LANGUAGE_EN | MASK_CASE_LOWER);

	/**
	 * Mode for inputing English upper characters with soft keyboard. 英文大写软键盘模弿
	 */
	 ...
	 * 丿个软键盘的切换状态的朿大数量?这里的切换状濁是指要显示的按键的状濿
	 * ，所以这些状态不可能有两个同时出现在丿个按键当中?如果有两个同时出现在一个按键当中，那就 不知道要显示哪一个了?
	 */
	 ...
	 * situation), {@link #mRecentLauageInputMode} will be tried. 前一个输入法的模弿
	 **/
	private int mPreviousInputMode = MODE_SKB_CHINESE;

	/**
	 * Used to remember recent mode to input language. 朿近的语言输入法模弿
	 */
	 ...
	 * 控制当前输入法模式软键盘布局要显示的按键切换状濁和要显示的行ID。比如当前软键盘布局丿
	 * ，有丿个按键有默认状濁?和两个切换状濁，ToggleStates中的mKeyStates[]保存的就是当前要显示的切换状怿
	 * ，如果按键的两个切换状濁都没有在mKeyStates[]丿
	 * ，那么按键就显示默认状濁，如果两个切换状濁中有一个在mKeyStates[]中，就显示那个切换状怿
	 * 。注意：绝对不可能有丿个按键的两个或两个以上的切换状濁同时在mKeyStates
	 * []中?ToggleStates不仅控制按键的显示状态，还可以直接控制一行按键的显示
	 * 。mRowIdToEnable保存的就是可显示的行的ID（每行的ID不是唯一皿 ，一个ID同时赋忼给多行）?只有ID丿
	 * mRowIdToEnable 咿 ALWAYS_SHOW_ROW_ID 的行才会被显示出来?
	 */
	private ToggleStates mToggleStates = new ToggleStates();

	/**
	 * The current field is a short message field? 当前的字段是否是丿个短语字段？ 彿
	...
	/**
	 * Is return key in normal state? 是否是正常状态下皿 Enter 键？
	 */
	...
	/**
	 * QWERTY row toggling state for URI input. 软键盘中的行的ID：Uri输入状濁?
	 */
	private int mToggleRowUri;

	/**
	 * QWERTY row toggling state for email address input. 软键盘中的行的ID：邮件地坿输入状濁?
	 */
	private int mToggleRowEmailAddress;

	/**
	 * 控制当前输入法模式软键盘布局要显示的按键切换状濁和要显示的行ID的管理类。比如当前软键盘布局丿
	 * ，有丿个按键有默认状濁?和两个切换状濁，ToggleStates中的mKeyStates[]保存的就是当前要显示的切换状怿
	 * ，如果按键的两个切换状濁都没有在mKeyStates[]丿
	 * ，那么按键就显示默认状濁，如果两个切换状濁中有一个在mKeyStates[]中，就显示那个切换状怿
	 * 。注意：绝对不可能有丿个按键的两个或两个以上的切换状濁同时在mKeyStates
	...
	class ToggleStates {
		/**
		 * If it is true, this soft keyboard is a QWERTY one. 是否是标准键盿
		 */
		boolean mQwerty;

		/**
		 * If {@link #mQwerty} is true, this variable is used to decide the
		 * letter case of the QWERTY keyboard. 是否是标准键盘大写模弿
		 */
		 ...
		 * details. 软键盘中要显示的行的ID。只有ID丿 mRowIdToEnable 咿 ALWAYS_SHOW_ROW_ID
		 * 的行才会被显示出来?
		 */
		 ...
		 * 这里保持此刻要显示的按键的状态，承以这些状态不可能有两个同时出现在丿个按键当中?如果有两个同时出现在一个按键当中，那就
		 * 不知道要显示哪一个了?
		 */
		public int mKeyStates[] = new int[MAX_TOGGLE_STATES];

		/**
		 * Number of states to toggle. 按键切换状濁的数量。mKeyStates[]的有用的长度?
		 */
	 ...
	 *      （中文符号软键盘丿模式）?MODE_SKB_SYMBOL2_CN（中文符号软键盘二模式）?
	 *      MODE_SKB_ENGLISH_LOWER
	 *      （英文小写软键盘模式）?MODE_SKB_ENGLISH_UPPER（英文大写软键盘模式
	 *      ）?MODE_SKB_SYMBOL1_EN（英文符号软键盘丿模式
	...
	/**
	 * 切换硬键盘的语言模式，返回切换后的语訿模式的图标?
	 * 先设置新的输入法语言是中文模式，再判断当前的输入法语訿模式是否是中文模式，是的话，就改成英文模式?
	 * 
	...
	public int requestBackToPreviousSkb() {
		...
		if (0 != layout && 0 != lastLayout) {
			// TODO 进行以下代码吿 mInputMode 咿 mPreviousInputMode 就一样了，这样好吗？
	...
	private void prepareToggleStates(boolean needSkb) {
		...
		// 根据EditorInfo.imeOptions添加 要显示的按键的切换状怿 ，以下只添加 Enter 键的切换状濁?
