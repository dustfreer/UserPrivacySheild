...
public class SkbContainer extends RelativeLayout implements OnTouchListener {
	 ...
	 * correction will be used. 对于手指触摸点的Y坐标进行的偏奿 ?
	 */
	...
	/**
	 * The input method service. 输入法服势
	 */
	...
	/**
	 * Used to indicate whether a popup soft keyboard is shown. 丿个弹出副的软件盘是否在显礿 ＿
	 */
	 ...
	 * 是否丿个副软键盘弹出框正在显示，并且等待触摸事件释放，触摸事件释放之后，副软键盘可以响应触摸事件？
	 **/
	private boolean mPopupSkbNoResponse = false;

	/** Popup sub keyboard. 副软键盘弹出桿 */
	 ...
	 * 当用户按下一个按键，丿个定时器启动，定时器时间到的时忙，霿要检查用户是否还按住这个键?
	 */
	...
	/**
	 * The x coordination of the last touch event. 朿后触摸事件的x坐标?
	 */
	private int mXLast;

	/**
	 * The y coordination of the last touch event. 朿后触摸事件的 y坐标?
	 */
	private int mYLast;

	/**
	 * The soft keyboard view. 软键盘视囿
	 */
	...
	/**
	 * The key pressed by user.用户按下的按锿
	 */
	private SoftKey mSoftKeyDown = null;

	/**
	 * Used to timeout a press if user holds the key for a long time. 长按定时噿
	 */
	...
	/**
	 * 切换候?词模式。?辑箿介：先从mInputModeSwitcher输入法模式交换器中获得中文忙?词模式状濁，然后判断是否是要切入候?词模式＿
	 * 如果是键盘就变为中文候?词模式状濿
	 * ，如果不是，键盘就消除中文忙?词模式状濁，变为mInputModeSwitcher中的mToggleStates设置键盘的状态?
	 * 
	...
	/**
	 * 更新输入法模式??辑箿介：先获取软键盘xml布局文件，然后更新软键盘布局，设置软键盘状濁?
	 */
	...
	/**
	 * 返回软键盘视图??辑箿介：先判断副软键盘弹出框是否在显示，是的话，就判断坐标点是否在副软键盘的区域内，如果是，就返回副软键盘弹出框＿
	...
	private void popupSymbols() {
		...
		if (popupResId > 0) {
			...
			SkbPool skbPool = SkbPool.getInstance();
			SoftKeyboard skb = skbPool.getSoftKeyboard(popupResId, popupResId,miniSkbWidth,miniSkbHeight, mContext);
			if (null == skb)
				return;
	...
	/**
	 * 副软键盘弹出框显示的时忙，如果realAction为true，那么就调用dismissPopupSkb（）隐藏副软键盘弹出框，显示主软键盘视图?
	 * 
	 * @param realAction
	 * @return
	 */
	...
	class LongPressTimer extends Handler implements Runnable {
		 ...
		 * generate first {@link #LONG_PRESS_KEYNUM1} key events. 长按时间丿
		 */
			...
			if (mWaitForTouchUp) {
				...
				if (mSoftKeyDown.repeatable()) {
					if (mSoftKeyDown.isUserDefKey()) {
						...
						// 系统定义的按键，长按相当于执行重复按键功能，mResponseTimes是按的次敿
						responseKeyEvent(mSoftKeyDown);
						long timeout;
